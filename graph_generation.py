...
# 2-2,
    #parralel: 
        ...
        #2.   2-1
            #1-1
            #1-2
# check for 1-1, if nothing is found above
# exclude in one function: Input: Graph, edge, indices_start; output: indices_end    
...
def add_features_and_predict_outcome(examples_to_test,  
                                             cat_to_explain,
                                             model,
                                             data,
                                             list_results,
    ...
    for _ in range(examples_to_test):
        ...
        try:
            ...
            print(255, 'in error: ', cat_to_explain, hd.x_dict)
    ...
    max_f1 = -1
    if compute_acc:
        tph,fph,fnh = compute_confusion_house(local_list[closest_index]) #old
    ...
    local_list[closest_index].append([max_acc, mean_acc, max_f1, tph,fph,fnh]) #accuracy does not change as here only different features are created
    list_results.append(local_list[closest_index])
